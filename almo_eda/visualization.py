...
def correlation_plot(qq_true, qq_pred, file_name="correlation.png", xylim=(-30, 0.5)):
    ...
    sc = plt.scatter(
        qq_true, qq_pred, c=z, cmap="plasma", s=22, edgecolor="none", alpha=0.5, label="Predictions"
    )
...
def loss_plot(train_losses, valid_losses, file_name="tv_loss.pdf"):
    ...
    plt.scatter(
        epochs[::50], train_losses[::50], color="C0", marker="o", edgecolor=None, label="_nolegend_"
    )

    # Plot the validation loss with a line and a square marker at every 10th epoch
    plt.scatter(
        epochs[::50], valid_losses[::50], color="C1", marker="s", edgecolor=None, label="_nolegend_"
    )
    ...
    plt.tight_layout()
    plt.savefig(file_name)
    logger.info(f"Loss plot saved as {file_name}")
    plt.close()

...
def plot_energy_histogram(data_out_donor_reshaped, num_bins=100, range_xax=(-30, 0), file_name="inp_data_hist.png"):
    """
    ...
    ax.hist(data_out_donor_reshaped[:, 0], bins=num_bins, range=range_xax, 
            weights=weights1, edgecolor="white", alpha=0.5, label="1st", color="red")
    ax.hist(data_out_donor_reshaped[:, 1], bins=num_bins, range=range_xax, 
            weights=weights2, edgecolor="white", alpha=0.4, label="2nd", color="blue")
...
def data_loader(n_snapshot, n_samples, n_features, output_type="donor", num_outputs=1,
                ...
                num_train_samples=500000, test_size=0.2, random_seed=321,
    ...
    #plot_energy_histogram(data_out_donor_reshaped, num_bins=100, range_xax=(-30, 0), file_name="out_data_hist_donor.png")
    #plot_energy_histogram(data_out_accep_reshaped, num_bins=100, range_xax=(-30, 0), file_name="out_data_hist_accep.png")

    # Convert to log scale
    data_out_log_accep = np.log(-data_out_accep_reshaped)
    data_out_log_donor = np.log(-data_out_donor_reshaped)

    #plot_energy_histogram(data_out_log_donor, num_bins=100, range_xax=(-4, 4), file_name="out_data_hist_donor_log.png")
    #plot_energy_histogram(data_out_log_accep, num_bins=100, range_xax=(-4, 4), file_name="out_data_hist_accep_log.png")
    # ==========================
...
#                    nn.init.zeros_(layer.bias)
    def _initialize_weights(self):
        """Initialize weights using He (Kaiming) initialization for better convergence with ReLU."""
        for layer in self.model:
            if isinstance(layer, nn.Linear):
...
def train_model(model, optimizer, train_loader, val_loader, criterion, device, num_epochs, early_stopping=False, patience=20):
    ...
    # Add scheduler
    #scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=10, min_lr=1e-6, verbose=True, threshold=1e-4)
...
def objective(trial):
    ...
    """
    # Randomly select the number of hidden layers (between 1 and 20)
    #num_layers = trial.suggest_int('num_layers', 1, 20)
    num_layers = trial.suggest_categorical('num_layers', [2, 4, 10, 20, 40])

    # Generate hidden layer sizes dynamically
    hidden_layer_sizes = [
        trial.suggest_int(f'hidden_layer_{i+1}', 50, 400, 50) for i in range(num_layers)
    ]

    # Choose activation function
    activation = trial.suggest_categorical('activation', [nn.ReLU, nn.LeakyReLU])

    # Use logarithmic sampling for learning rate to improve search space efficiency
    ...
    model = FFNet(input_size=n_features, hidden_layers=hidden_layer_sizes, output_size=2, activation_func=activation)
    model.to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay_param)

    val_loss = train_model(model, optimizer, train_loader, val_loader, criterion, device, num_epochs, early_stopping=True, patience=10)
...
if __name__ == "__main__":
    ...
    n_features, var_energy_train, D_train, D_valid, E_train, E_valid = data_loader(n_snapshot, n_samples, n_features, output_type="donor", num_outputs=output_size, start_index=90000, end_index=96000, step=2, num_train_samples=375000, random_seed=random_seed)
